...
def render_chart(data: dict[str, pd.DataFrame], summary: dict[str, float | int]) -> None:
    ...
    cost_df["avg_recovery_rate"] = pd.to_numeric(
        cost_df["avg_recovery_rate"], errors="coerce"
    )
    cost_df["data_points"] = pd.to_numeric(cost_df["data_points"], errors="coerce")
    cost_df["recovery_per_1k_usd"] = pd.to_numeric(
        cost_df["recovery_per_1k_usd"], errors="coerce"
    )
    
    # Remove rows with missing critical values
    cost_df = cost_df.dropna(
        subset=["avg_cost_usd", "avg_recovery_rate"]
    )
    ...
    with st.expander("📊 Cost-Effectiveness Summary"):
        ...
        top_efficient.columns = ["Disease", "Country", "Avg Cost ($)", "Recovery Rate (%)", "Recovery per $1k"]
        st.dataframe(
            top_efficient,
